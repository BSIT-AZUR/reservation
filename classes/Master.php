...
class Master extends DBConnection
{
	...
	function save_accommodation()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "New Time successfully saved.");
		} else {
			$resp['status'] = 'failed';
	...
	function delete_room()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			if (is_dir(base_app . 'uploads/room_list' . $id)) {
				$file = scandir(base_app . 'uploads/therapy_' . $id);
				foreach ($file as $img) {
					if (in_array($img, array('..', '.')))
						continue;
					unlink(base_app . 'uploads/therapy_' . $id . '/' . $img);
				}
				rmdir(base_app . 'uploads/therapy_' . $id);
			}
			$this->settings->set_flashdata('success', "asasasSuccessfully deleted.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_room()
	{
		...
		$check = $this->conn->query("SELECT * FROM `booking` where room_id = '{$room_id}' and ((date('{$check_in}') BETWEEN `check_in` and `check_out`) or (date('{$check_out}') BETWEEN `check_in` and `check_out`)) and `status` in (0,1) ")->num_rows;
		if($check > 0){
			$resp['status'] = "failed";
		...
		if ($save) {
			if (isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0) {
				if (!is_dir(base_app . 'uploads/therapy_' . $id)) {
					mkdir(base_app . 'uploads/therapy_' . $id);
					$data = " `upload_path`= 'uploads/therapy_{$id}' ";
				} else {
					$data = " `upload_path`= 'uploads/therapy_{$id}' ";
				}
				$this->conn->query("UPDATE `room_list` set {$data} where id = '{$id}' ");
				foreach ($_FILES['img']['tmp_name'] as $k => $v) {
					move_uploaded_file($_FILES['img']['tmp_name'][$k], base_app . 'uploads/therapy_' . $id . '/' . $_FILES['img']['name'][$k]);
				}
			}

			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "New Therapy successfully saved.");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
